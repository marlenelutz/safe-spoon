...
PURITY_FACTOR = _au_cfg["purity_factor"]   # max_purity = PURITY_FACTOR / K
...
def api_annotation_units(cat):
    ...
    # Derive max_purity from the number of topics K so the threshold scales automatically when different domains have different K values.
    k = thetas_for_tree.shape[1]
    max_purity = round(min(PURITY_FACTOR / k, 0.80), 4)
    ...
    payload = {
        ...
        "topic_keys": info.get("topic_keys",   []),
        "topic_labels": info.get("topic_labels", []),
        "config":{"min_size": MIN_SIZE, "max_purity": max_purity, "purity_factor": PURITY_FACTOR, "k": k},
    }
