...
class SimpleTMPreprocessor:
    """Lightweight text preprocessor for topic modeling."""
    ...
    def __init__(
        ...
        *,
        ...
        self.valid_pos = set(valid_pos) if valid_pos else {"NOUN", "VERB", "ADJ", "PROPN"}
    ...
    def _load_stopwords(files: List[str]) -> set:
        ...
        for f in files:
            ...
            if not p.exists():
                continue
            out |= {w.strip().lower() for w in p.open("r", encoding="utf8").readlines()}
        return out
    ...
    def _load_equivalents(files: List[str]) -> dict:
        ...
        for f in files:
            ...
            if p.suffix.lower() == ".json":
                lines = json.load(p.open("r", encoding="utf8")).get("wordlist", [])
            else:
    ...
    def fit_transform(
        ...
        out = pd.DataFrame()
        out["id"] = df[id_col].values if id_col in df.columns else range(len(df))
        ...
        if compute_tfidf:
            if self._cv is None:
                raise RuntimeError("compute_bow must be True before compute_tfidf.")
